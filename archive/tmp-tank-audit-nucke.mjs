...
async function main() {
  ...
  console.log(`${PLAYER} (Vengeance DH Tank) â€” sourceID: ${playerId} â€” ${fights.length} pulls`);
    ...
    if (isBest) {
      ...
      for (const spell of defensiveSpells) {
        ...
        if (uses.length > 0) {
          ...
          console.log(`      ${spell.padEnd(22)} ${uses.length}x â€” ${times}`);
        }
      }
    }
  }
  ...
  if (earlyDeaths.length > 0) {
    ...
    for (const p of earlyDeaths) {
      console.log(`    #${p.id} at ${fmt(p.earlyDeath.time)} â€” ${p.earlyDeath.ability}`);
  ...
  for (let i = 0; i < Math.min(3, rankings.length); i++) {
    ...
    console.log(`  #${i+1} ${r.name} â€” ${topDPS.toLocaleString()} DPS, ${topDTPS.toLocaleString()} DTPS (${fmt(topDur)})`);
