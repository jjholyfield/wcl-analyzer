...
const SID_OVERRIDE = process.argv[5] ? parseInt(process.argv[5]) : null;
const DPS_POT_KEYWORDS = ['recklessness', "light's potential", 'zealotry', 'rampant abandon'];

if (!PLAYER || !CLASS || !SPEC) {
  console.log('Usage: node tmp-healer-audit.mjs <name> <class> <spec> [sourceID]');
  process.exit(1);
}
