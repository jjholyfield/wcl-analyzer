...
async function main() {
  ...
  if (earlyDeaths.length > 0) {
    console.log('\n  Deaths:');
    for (const p of earlyDeaths) console.log(`    #${p.id} at ${fmt(p.earlyDeath.time)} — ${p.earlyDeath.ability}`);
  }
  ...
  for (let i = 0; i < Math.min(3, rankings.length); i++) {
    ...
    console.log(`  #${i+1} ${r.name} — ${topHPS.toLocaleString()} HPS, ${topCPM} CPM, ${topOH}% OH (${fmt(topDur)})`);
